...
mod tests {
    ...
    fn test_cpu_affinity_manager_creation() {
        ...
        assert!(manager.topology().total_cpus > 0);
        assert!(!manager.topology().numa_nodes.is_empty());
    }

    #[test]
    fn test_worker_assignment() {
        let mut manager = CpuAffinityManager::default();
        let assignments = manager.assign_workers(4).unwrap();
        
        assert_eq!(assignments.len(), 4);
        
        // Check that all assignments have valid CPU IDs
        for assignment in &assignments {
            assert!(assignment.cpu_id < manager.topology().total_cpus);
        }
    }
    ...
    fn test_cpu_list_parsing() {
        // Test simple range
        let cpus = CpuAffinityManager::parse_cpu_list("0-3").unwrap();
        assert_eq!(cpus, vec![0, 1, 2, 3]);
        
        // Test complex range
        let cpus = CpuAffinityManager::parse_cpu_list("0-1,4-5").unwrap();
        assert_eq!(cpus, vec![0, 1, 4, 5]);
        
        // Test single CPU
        let cpus = CpuAffinityManager::parse_cpu_list("7").unwrap();
        assert_eq!(cpus, vec![7]);
    }

    #[test]
    fn test_performance_recommendations() {
        let manager = CpuAffinityManager::default();
        let recommendations = manager.get_performance_recommendations(1);
        
        assert!(!recommendations.is_empty());
        assert!(recommendations.iter().any(|r| r.contains("Single worker")));
    }

    #[test]
    fn test_optimal_cpu_list() {
        let manager = CpuAffinityManager::default();
        let cpus = manager.get_optimal_cpu_list();
        
        assert!(!cpus.is_empty());
        assert!(cpus.len() <= manager.topology().total_cpus);
        
        // Should be sorted
        let mut sorted_cpus = cpus.clone();
        sorted_cpus.sort();
        assert_eq!(cpus, sorted_cpus);
    }
}