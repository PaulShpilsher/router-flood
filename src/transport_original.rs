//! Transport channel management and optimization
//!
//! This module provides optimized transport channel creation and management,
//! implementing per-worker channels to eliminate mutex contention.
...
/// Transport channel types for different protocol layers
...
impl WorkerChannels {
    ...
    fn send_ipv4_packet(&mut self, packet_data: &[u8], target_ip: IpAddr) -> Result<()> {
        if let Some(ref mut tx) = self.ipv4_sender {
            ...
            tx.send_to(packet, target_ip)
                .map_err(|e| NetworkError::PacketSend(format!("IPv4 send failed: {}", e)))?;
    ...
    fn send_ipv6_packet(&mut self, packet_data: &[u8], target_ip: IpAddr) -> Result<()> {
        if let Some(ref mut tx) = self.ipv6_sender {
            ...
            tx.send_to(packet, target_ip)
                .map_err(|e| NetworkError::PacketSend(format!("IPv6 send failed: {}", e)))?;
    ...
    fn send_l2_packet(&mut self, packet_data: &[u8]) -> Result<()> {
        if let Some(ref mut tx) = self.l2_sender {
            match tx.send_to(packet_data, None) {
                Some(Ok(_)) => Ok(()),
                Some(Err(e)) => Err(NetworkError::PacketSend(format!("L2 send failed: {}", e)).into()),
                None => Err(NetworkError::PacketSend("L2 sender not available".to_string()).into()),
            }
        } else {
    ...
    /// Check if channels are available (for dry-run mode detection)
    pub fn has_channels(&self) -> bool {
        self.ipv4_sender.is_some() || self.ipv6_sender.is_some() || self.l2_sender.is_some()
    }
}
