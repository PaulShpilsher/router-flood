...
mod tests {
    ...
    async fn test_simple_worker() {
        ...
        let mut worker = SimpleWorker::new(
            0,
            stats.clone(),
            packet_builder,
            target_provider,
    ...
    #[test]
    fn test_simple_worker_factory() {
        let config = Arc::new(MockWorkerConfig);
        let factory = SimpleWorkerFactory::new(config);
        
        let stats = Arc::new(MockStatsCollector::new());
        let packet_builder = Box::new(MockPacketBuilder);
        let target_provider = Arc::new(MockTargetProvider::new(vec![80]));
        
        let worker = factory.create_worker(0, stats, packet_builder, target_provider);
