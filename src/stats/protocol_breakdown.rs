...
mod tests {
    ...
    #[test]
    fn test_protocol_index_conversion() {
        assert_eq!(ProtocolIndex::from_protocol_name("UDP"), Some(ProtocolIndex::Udp));
        assert_eq!(ProtocolIndex::from_protocol_name("TCP"), Some(ProtocolIndex::Tcp));
        assert_eq!(ProtocolIndex::from_protocol_name("INVALID"), None);
        
        assert_eq!(ProtocolIndex::Udp.to_protocol_name(), "UDP");
        assert_eq!(ProtocolIndex::Tcp.to_protocol_name(), "TCP");
    }
    
    #[test]
    fn test_protocol_breakdown() {
        let mut breakdown = ProtocolBreakdown::new();
        
        // Test increment by name
        breakdown.increment("UDP");
        breakdown.increment("TCP");
        breakdown.increment("UDP");
        
        assert_eq!(breakdown.get("UDP"), 2);
        assert_eq!(breakdown.get("TCP"), 1);
        assert_eq!(breakdown.get("ICMP"), 0);
        
        // Test increment by index
        breakdown.increment_by_index(ProtocolIndex::Icmp);
        assert_eq!(breakdown.get("ICMP"), 1);
        
        // Test total
        assert_eq!(breakdown.total(), 4);
        
        // Test HashMap conversion
        let hashmap = breakdown.to_hashmap();
        assert_eq!(hashmap.get("UDP"), Some(&2));
        assert_eq!(hashmap.get("TCP"), Some(&1));
        
        let from_hashmap = ProtocolBreakdown::from_hashmap(&hashmap);
        assert_eq!(from_hashmap.get("UDP"), 2);
        assert_eq!(from_hashmap.get("TCP"), 1);
    }
    
    #[test]
    fn test_protocol_breakdown_merge() {
        let mut breakdown1 = ProtocolBreakdown::new();
        breakdown1.increment("UDP");
        breakdown1.increment("TCP");
        
        let mut breakdown2 = ProtocolBreakdown::new();
        breakdown2.increment("UDP");
        breakdown2.increment("ICMP");
        
        breakdown1.merge(&breakdown2);
        
        assert_eq!(breakdown1.get("UDP"), 2);
        assert_eq!(breakdown1.get("TCP"), 1);
        assert_eq!(breakdown1.get("ICMP"), 1);
        assert_eq!(breakdown1.total(), 4);
    }
    
    #[test]
    fn test_protocol_breakdown_display() {
        let mut breakdown = ProtocolBreakdown::new();
        breakdown.increment("UDP");
        breakdown.increment("TCP");
        breakdown.increment("UDP");
        ...
        assert!(display.contains("UDP: 2"));
        assert!(display.contains("TCP: 1"));
        assert!(!display.contains("ICMP"));
    }
}